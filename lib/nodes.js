...
Nodes.prototype = {
  ...
  'always': function(doc){
    ...
    if (doc.location) {
      this.aggregateLocation(doc.location);
    }
  ...
  'aggregateLocation': function(location){
    this.locationTotals[location]++;
    this.locationTotals.All++;
    this.aggregator.data.InteractionMap.locationId = this.locationMap[location];
